 ...
 * 
 * Features:
 * - Multi-tenant isolation by user_phone (Customer A and Customer B data isolated)
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      await dbRun(
        `UPDATE users SET username = ?, business_name = ?, business_type = ?, address = ?, role = ?, password_hash = ?, device_id = ?, devices = ?, status = 'on', start_date = ?, end_date = ? WHERE phone_no = ?`,
        [username, businessName || '', businessType || '', address || '', role || 'ADMIN', password, deviceId || existing.device_id || '', updatedDevicesJson, startStr, trialEndStr, phoneNo]
      );

      return res.json({
        success: true,
        message: 'Registration updated (7 Days Trial Active)',
        status: 'on',
        user: { phoneNo, username, businessName, businessType, address, role, deviceId, status: 'on', startDate: startStr, endDate: trialEndStr }
      });
    }
    ...
    await dbRun(
      `INSERT INTO users (phone_no, username, business_name, business_type, address, role, password_hash, device_id, devices, status, start_date, end_date, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'on', ?, ?, ?)`,
      [phoneNo, username, businessName || '', businessType || '', address || '', role || 'ADMIN', password, deviceId || '', devicesJson, startStr, trialEndStr, now]
    );
...
app.post('/api/check-status', async (req, res) => {
  try {
    ...
    const devCheck = parseDevices(user, deviceId);
    if (!devCheck.allowed) {
      return res.json({ success: true, status: 'device_mismatch', message: 'Max 5 Devices Limit Reached' });
    }
...
// 4. GET ALL USERS (Admin Check)
app.get('/api/users', async (req, res) => {
  try {
    const users = await dbAll('SELECT phone_no as phoneNo, username, business_name as businessName, business_type as businessType, address, role, device_id as deviceId, devices, status, start_date as startDate, end_date as endDate, created_at FROM users');
...
// 👉 4.1 UPDATE USER (Ban, Block, Change Expire Date - New Code)
...
// 👉 4.2 DELETE USER (New Code)
app.delete('/api/users/:phoneNo', async (req, res) => {
  try {
    const phoneNo = req.params.phoneNo;
    await dbRun('DELETE FROM users WHERE phone_no = ?', [phoneNo]);
    // Optional: Delete user's isolated data here if you want full wipe, 
    // but keeping it soft is safer. Admin can wipe using other APIs.
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    let products = [];
    if (uPhone) {
      products = await dbAll('SELECT *, user_phone as userPhone FROM products WHERE user_phone = ? OR user_phone IS NULL', [uPhone]);
    } else {
      products = await dbAll('SELECT *, user_phone as userPhone FROM products');
    }
...
// 8. VOUCHERS API (ISOLATED BY USER PHONE)
